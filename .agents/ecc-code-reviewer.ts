/**
 * ECC Code Reviewer — Integrated into MetaBuff Ecosystem
 *
 * Superior code review specialist with confidence-based filtering,
 * pre-report gate validation, and structured severity scoring.
 * Complements MetaBuff's existing reviewer with richer quality gates.
 *
 * Source: ECC (affaan-m/ECC) agents/code-reviewer.md
 */
...
const definition: AgentDefinition = {
  ...
  systemPrompt:
    ...
    'You apply confidence-based filtering — only report issues you are >80% confident about. ' +
    'A clean review with zero findings is a valid and expected outcome. ' +
    'You use a structured severity system (CRITICAL/HIGH/MEDIUM/LOW) with explicit pre-report gates.',
...
- **"Missing await"** on fire-and-forget calls that are intentionally detached,
  such as logging, metrics, or background queue pushes.
...
- **Security theater**: flagging \`Math.random()\` in a non-cryptographic context
  such as animation, jitter, or sampling.
...
## Approval Criteria
- **Approve**: No CRITICAL or HIGH issues. Clean reviews with zero findings are valid.
