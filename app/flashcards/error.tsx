...
export default function FlashcardsError({
  ...
  useEffect(() => {
    console.error('Flashcards page error:', error);
  }, [error]);
    ...
    <main className="min-h-screen flex items-center justify-center p-6" style={{ backgroundColor: 'var(--editorial-bg)', color: 'var(--editorial-text)' }}>
      <div className="max-w-md w-full text-center space-y-8">
        <div className="space-y-4">
          ...
          <h1 className="text-2xl font-bold" style={{ color: 'var(--editorial-text)' }}>Unable to Load Flashcards</h1>
          <p className="text-sm leading-relaxed" style={{ color: 'var(--editorial-muted)' }}>
            Something went wrong while loading the flashcard deck. This is usually temporary.
          </p>
        </div>
